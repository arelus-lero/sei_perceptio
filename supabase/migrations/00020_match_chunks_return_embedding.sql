-- Migration: 00020_match_chunks_return_embedding.sql
-- Retorna embedding pré-calculado dos chunks para verificação de confiança (sem re-embed)

DROP FUNCTION IF EXISTS public.match_chunks_hybrid(vector, text, uuid, uuid[], text[], text[], date, date, text, text, uuid[], integer, integer, integer);
...
AS $$
  WITH filtered_chunks AS (
    ...
    WHERE f.notebook_id = p_notebook_id
      ...
      AND (
        p_interessado IS NULL
        OR trim(p_interessado) = ''
        OR EXISTS (
          SELECT 1
          FROM public.processo pr
          WHERE pr.orgao_id = f.orgao_id
            AND (
              (
                f.documento_sei_id IS NOT NULL
                AND pr.id = (
                  SELECT d.processo_id
                  FROM public.documento d
                  WHERE d.id = f.documento_sei_id
                  LIMIT 1
                )
              )
              OR (
                NULLIF(c.metadados_json ->> 'nup', '') IS NOT NULL
                AND pr.nup = c.metadados_json ->> 'nup'
              )
            )
            AND EXISTS (
              SELECT 1
              FROM jsonb_array_elements(pr.interessados) AS elem
              WHERE lower(COALESCE(elem ->> 'nome', '')) LIKE '%' || lower(trim(p_interessado)) || '%'
                OR (
                  length(regexp_replace(trim(p_interessado), '\D', '', 'g')) >= 3
                  AND regexp_replace(
                    COALESCE(
                      elem ->> 'documento',
                      elem ->> 'cpf',
                    ...
                    '',
                    'g'
                  ) LIKE '%' || regexp_replace(trim(p_interessado), '\D', '', 'g') || '%'
                )
            )
        )
      )
  ...
  SELECT
    ...
    fc.embedding
  FROM rrf_scores rs
  INNER JOIN filtered_chunks fc ON fc.id = rs.id
  ORDER BY rs.score_rrf DESC
  LIMIT p_top_k;
$$;

REVOKE ALL ON FUNCTION public.match_chunks_hybrid(vector, text, uuid, uuid[], text[], text[], date, date, text, text, uuid[], integer, integer, integer) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.match_chunks_hybrid(vector, text, uuid, uuid[], text[], text[], date, date, text, text, uuid[], integer, integer, integer) TO authenticated, service_role;
